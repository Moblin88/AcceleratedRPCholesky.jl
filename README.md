...
# Returns a dynamically sized factor G and pivot indices
...
### `rpcholesky(kernel, data; rank=min(n, 50), rtol=0.05, atol=1e-8, block_size=clamp(round(Int, √n), 4, 24))`
...
- `G` is an `n × k` matrix, where `k = length(pivots)` is the accepted rank.
...
G, pivots = rpcholesky(rbf, data)
G, pivots = rpcholesky(rbf, data; rank=30, rtol=0.01)
```
