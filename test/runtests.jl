...
@testset "AcceleratedRPCholesky.jl" begin
    ...
    @testset "rpcholesky - kernel interface" begin
        ...
        @testset "supports DataFrames" begin
            ...
            G, pivots = rpcholesky(dataframe_rbf, data, 5)
        ...
        @testset "preserves Float32 kernel type" begin
            ...
            G, pivots = rpcholesky(rbf32, X32, 5)
        ...
        @testset "approximation quality" begin
            G, pivots = rpcholesky(rbf, X, n)
            approx = G * G'
            err = norm(K - approx, 1) / norm(K, 1)
            @test err < 0.5
        end

        @testset "optional positional rank defaults to min(n, 50)" begin
        ...
        @testset "block_size parameter accepted" begin
            G, pivots = rpcholesky(rbf, X, 3; block_size=2)
            @test size(G, 2) <= 3
            @test length(pivots) <= size(G, 2)
        end

        @testset "atol stopping" begin
            Random.seed!(42)
            G_tight, _ = rpcholesky(rbf, X, n)
            Random.seed!(42)
            G_loose, _ = rpcholesky(rbf, X, n; atol=1e-2)
            @test size(G_loose, 2) <= size(G_tight, 2)
        end

        @testset "residual trace tolerance" begin
            Random.seed!(42)
            G, pivots = rpcholesky(rbf, X, n; atol=1e-8, block_size=4)
            residual = K - G * G'
            @test tr(Symmetric(residual)) <= 1e-8 + 1e-8
        end

        @testset "zero kernel returns empty factor" begin
            zero_kernel(x, y) = 0.0
            G, pivots = rpcholesky(zero_kernel, X, 5)
            @test G isa Matrix
            @test all(iszero.(G))
        ...
        @testset "rank greater than block size" begin
            Random.seed!(42)
            G, pivots = rpcholesky(rbf, X, 8; block_size=1)
            @test size(G, 1) == n
            @test size(G, 2) > 1
            @test length(pivots) <= size(G, 2)
            approx = G * G'
        ...
        @testset "rank cap on accepted pivots" begin
            ...
            Random.seed!(42)
            G, pivots = rpcholesky(rbf, X, 2; block_size=6)
            @test size(G, 2) <= 2
            @test length(pivots) <= size(G, 2)
        end
        ...
        @testset "small final block (b < block_size)" begin
            Random.seed!(42)
            G, pivots = rpcholesky(rbf, X, 7; block_size=4)
            @test size(G, 2) <= 7
            @test length(pivots) == size(G, 2)
        end
    end
